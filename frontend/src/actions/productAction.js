...
export const getAdminProduct = () => async(dispatch) => {
    try {
        ...
        const {data} = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/v1/admin/products`, {
            headers:{
...
export const createProduct = (productData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/v1/admin/product/new`, productData, {
        headers: {
            token: localStorage.getItem("token"),
  ...
  export const updateProduct = (id, productData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/v1/admin/product/${id}`, productData, {
        headers: {
            token: localStorage.getItem("token"),
            "Content-Type": "application/json"
        }
  ...
  export const deleteProduct = (id) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.delete(`${process.env.REACT_APP_BASE_URL}/api/v1/admin/product/${id}`, {
        headers: {
            token: localStorage.getItem("token"),
        }
      });
...
export const newReview = (reviewData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(`${process.env.REACT_APP_BASE_URL}/api/v1/review`, reviewData, {
        headers: {
            token: localStorage.getItem("token"),
            "Content-Type": "application/json"
        }
